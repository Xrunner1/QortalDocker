...
if [ ! -z "${EXISTING_RUNNING_CONTAINER_ID}" ]; then
    ...
    docker exec -it "${EXISTING_RUNNING_CONTAINER_ID}" curl -X DELETE "http://localhost:${API_PORT}/admin/mintingaccounts" -H "accept: text/plain" -H "Content-Type: text/plain" -d "${MINTING_KEY}"
    echo
    echo "Minting accounts:"
    docker exec -it "${EXISTING_RUNNING_CONTAINER_ID}" curl "http://localhost:${API_PORT}/admin/mintingaccounts" | jq
else
