...
if [ ! -z "${EXISTING_RUNNING_CONTAINER_ID}" ]; then
    ...
    docker exec -it "${EXISTING_RUNNING_CONTAINER_ID}" curl -X POST "http://localhost:${API_PORT}/admin/mintingaccounts" -H "accept: text/plain" -H "Content-Type: text/plain" -d "${MINTING_KEY}"
    echo
